...
- Обработка файлов вывода для улучшения читаемости
...
def process_file(path: Path):
    """
    Обрабатывает файл вывода, удаляя лишние строки после маркера статистики.

    ВХОД:
        path (Path): Путь к файлу для обработки

    ВЫХОД:
        None (файл модифицируется на месте)
    """
...
def show_finger_stats(analyzer,
    ...
    ВХОД:
        ...
        layout_name (str, optional): Имя раскладки для анализа
                                   ...
                                   (по умолчанию "qwer")
        output_file (Optional[str]): Путь к файлу для записи вывода
    ...
    Действия функции:
        ...
        - Обрабатывает файл вывода для улучшения читаемости
