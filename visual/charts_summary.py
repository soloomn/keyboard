...
- Создание одной круговой диаграммы с суммарной нагрузкой по раскладкам
- Автоматический расчет процентного соотношения
- Выделение сегментов диаграммы для улучшения визуального восприятия
- Интеллектуальное размещение процентных меток
...
ВХОД:
    Модуль принимает данные через функцию create_total_load_pie_chart
    в виде трёх словарей с данными о нагрузках для каждой раскладки.

ВЫХОД:
    Модуль не возвращает значения, но сохраняет график в файл
    '/app/data_output/charts_summary.png'
"""
...
def create_total_load_pie_chart(data_diktor: dict,
    ...
    Функция визуализирует суммарную нагрузку для трёх клавиатурных раскладок
    в виде круговой диаграммы с автоматическим расчётом процентного соотношения
    и интеллектуальным размещением меток.
    ...
    Побочные эффекты:
        - Создаёт и сохраняет круговую диаграмму в файл '/app/data_output/charts_summary.png'
        - В случае отсутствия данных выводит сообщение "Нет данных"


    Действия функции:
        - Рассчитывает общую нагрузку для каждой раскладки (сумма левой и правой руки)
        - Создает круговую диаграмму с тремя сегментами
        - Автоматически вычисляет процентное соотношение
        - Выделяет сегмент "ВЫЗОВ" для акцента
        - Интеллектуально размещает процентные метки с учетом позиции сегментов
        - Сохраняет график в файл с разрешением 300 DPI
        - Обрабатывает случай отсутствия данных (отображает сообщение "Нет данных")
    """
    values = [
