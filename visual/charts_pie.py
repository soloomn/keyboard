...
ВХОД:
    Модуль принимает данные через функции plot_only_pie_charts или plot_all_pie_charts
    в виде словарей с данными о нагрузках для каждой раскладки.

ВЫХОД:
    Модуль не возвращает значения, но сохраняет графики в файл
    '/app/data_output/charts_pie.png'
"""
...
def create_pie_chart(ax, values_left, values_right, values_two_handed,
    ...
    ВХОД:
        ax (matplotlib.axes.Axes): Объект осей matplotlib для отрисовки диаграммы
        ...
        labels_base (list): Список базовых меток для сегментов ['Л. Рука', 'П. Рука', 'Двуручие']
        title (str): Заголовок диаграммы
        explode (tuple): Кортеж из 3 элементов для выделения сегментов
                         (0 - без выделения, >0 - с выделением)

    ВЫХОД:
        matplotlib.axes.Axes: Объект осей с построенной диаграммой
