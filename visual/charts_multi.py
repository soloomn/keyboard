...
ВХОД:
    Модуль принимает данные через функцию plot_finger_loads_by_layout в виде
    7 словарей с данными раскладок.

ВЫХОД:
    Модуль не возвращает значения, но сохраняет график в файл
    '/app/data_output/charts_multi.png'
"""
...
def plot_finger_loads_by_layout(data_diktor: dict,
    ...
    Функция создает 5 графиков (2x3 сетка) - по одному на каждый тип пальца.
    На каждом графике отображает нагрузки для левой (Л) и правой (П) руки
    и сравнивает 7 раскладок клавиатуры разными цветами.
    ...
    ВХОД:
        ...
        data_skoropis (dict): Данные для раскладки "Скоропись"
        data_zubachew (dict): Данные для раскладки "Зубачев"
        data_rusphone (dict): Данные для раскладки "РусФон"
